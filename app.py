...
def analyse_sentiment(headlines: list[dict]) -> dict:
    ...
    Strategy:
      1. If TextBlob is available → use its polarity on each headline.
      2. Otherwise → simple positive/negative keyword counting.

    Returns:
        {
          "score":   float,          # aggregate [-1, +1]
          "label":   str,            # "Positive" / "Negative" / "Neutral"
          "scores":  list[float],    # per-headline polarity
          "method":  str,            # "TextBlob" | "Keyword"
        }
    """
    if not headlines:
...
def run_pipeline(ticker, start_str, end_str, initial_cap,
    ...
    def _normalise(d):
        if d is None or (hasattr(d, "empty") and d.empty):
            return None
        d = d.copy()
        # Flatten MultiIndex columns → take first level (field names)
        ...
        d.index.name = "Date"
        # Flatten any remaining 2-D columns to 1-D
    ...
    # A: Ticker.history() — most reliable across all yfinance versions
    try:
    ...
    # B: yf.download with multi_level_index=False (yfinance ≥ 0.2.50)
    if raw is None:
        try:
            _d = yf.download(
    ...
    # C: plain yf.download (older yfinance)
    if raw is None:
        try:
            _d = yf.download(
                ticker, start=start_str, end=end_str,
    ...
    # D: period="max" then trim (last resort)
    if raw is None:
        try:
    ...
    # ── Coerce Close & Volume to clean float Series ───────────────────────────
    ...
    # 6. Next-day signal  (sentiment injected externally — see below)
    latest_x      = X[-1:].copy()
    # If News_Sentiment is in feats, find its column index and inject the
    # current sentiment score at prediction time (handled after pipeline returns).
...
_lp_sentiment = R["latest_prob"]   # default: original probability
if enable_sentiment and R["sentiment_feat_idx"] is not None and _headlines:
    try:
        import sklearn   # already imported, just need the model reference
        # We re-run predict_proba with the sentiment-injected feature vector.
        # The model object is NOT in R (cached without it to save memory), so
        # we blend the probability heuristically:
        #   new_prob = clamp(old_prob + 0.5 * sentiment_score * weight, 0, 1)
        # Weight is deliberately small (0.15) so sentiment nudges, not dominates.
        SENT_WEIGHT = 0.15
...
# ██  VERDICT BANNER  ██
# ─────────────────────────────────────────────────────────
lp  = _lp_sentiment          # sentiment-adjusted probability
ls  = _ls_sentiment
...
# Score: +1 bullish / -1 bearish per indicator
score = 0
reasons = []
if lp > R["buy_thresh"]:
...
# ── Sentiment signal (Requirement 6) ─────────────────────────────────────
if enable_sentiment and _headlines:
    ...
    MAX_SCORE = 8    # added 1 for sentiment
else:
    MAX_SCORE = 7
...
# Sentiment label for banner sub-line
_sent_badge = ""
if enable_sentiment and _headlines:
...
# ─────────────────────────────────────────────────────────
# 📰 NEWS SENTIMENT PANEL  (Requirement 7)
# ─────────────────────────────────────────────────────────
if enable_sentiment:
